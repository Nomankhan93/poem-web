...
export default async function AdminCareersPage({
  ...
  const { data: items } = await supabase
    .from("careers")
    .select("id,title,department,status,published,updated_at")
  ...
  return (
    <div>
      <AdminPageHeader
        eyebrow="Content"
        title="Careers"
        description="Manage POEM careers content from the administration workspace."
        action={
          <Link href="/admin/careers/new" className="inline-flex items-center gap-2 rounded-full bg-poem-950 px-5 py-3 text-sm font-extrabold text-white">
            New vacancy
      ...
      {params.saved ? <Notice>Vacancy saved.</Notice> : null}
      {params.updated ? <Notice>Vacancy unpublished.</Notice> : null}
      ...
      <div className="mt-7 overflow-hidden rounded-[24px] border border-black/[0.06] bg-white">
        <div className="divide-y divide-black/5">
          {(items ?? []).map((item) => (
            <div key={item.id} className="flex flex-col gap-4 px-5 py-5 md:flex-row md:items-center md:justify-between">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <Link href={`/admin/careers/${item.id}`} className="font-extrabold text-poem-950 hover:text-poem-700">
                    {item.title}
                  </Link>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Link href={`/admin/careers/${item.id}`} className="rounded-full border border-black/10 px-4 py-2 text-xs font-extrabold text-poem-900">
                  Edit
                </Link>

                {item.published ? (
                  <form action={unpublishItem}>
                    <input type="hidden" name="table" value="careers" />
                    <input type="hidden" name="id" value={item.id} />
                    <input type="hidden" name="return_to" value="/admin/careers" />
          ...
          {!items?.length ? (
            <div className="px-6 py-12 text-center text-sm text-poem-muted">No careers yet.</div>
          ) : null}
        </div>
      </div>
    </div>
